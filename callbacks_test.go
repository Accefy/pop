...
func Test_Callbacks_on_Slice(t *testing.T) {
	...
	transaction(func(tx *Connection) {
		r := require.New(t)
		for i := 0; i < 2; i++ {
			r.NoError(tx.Create(nil, &CallbacksUser{}))
		}

		users := CallbacksUsers{}
		r.NoError(tx.All(nil, &users))
		r.Len(users, 2)
		for _, u := range users {
		...
		r.NoError(tx.Eager().All(nil, &users))
		r.Len(users, 2)
		for _, u := range users {
